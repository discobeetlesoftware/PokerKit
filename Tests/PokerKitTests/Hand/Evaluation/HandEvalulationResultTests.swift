...
class HandEvaluationResultTests: XCTestCase {
    func testInit() {
        let result1 = HandEvaluationResult()
        XCTAssertNotNil(result1)
        XCTAssertNil(result1.rank)
        XCTAssertNil(result1.primary)
        XCTAssertNil(result1.kickers)

        let primary:Hand? = PlayingCard.hand("ah ac")
        let kickers:Hand? = PlayingCard.hand("2c 3c 4c")
    ...
    func testEquatable_primary() {
        let pairDeuces1 = HandEvaluationResult(rank: .pair, primary: PlayingCard.hand("2c 2s"), kickers: nil)
        let pairDeuces2 = HandEvaluationResult(rank: .pair, primary: PlayingCard.hand("2c 2s"), kickers: nil)
        ...
        let trips1 = HandEvaluationResult(rank: .threeOfKind, primary: PlayingCard.hand("3c 3h 3s"), kickers: nil)
        let trips2 = HandEvaluationResult(rank: .threeOfKind, primary: PlayingCard.hand("3c 3h 3d"), kickers: nil)
        XCTAssertNotEqual(trips1, trips2)
    }

    func testEquatable_kickers() {
        let pair1 = HandEvaluationResult(rank: .pair, primary: PlayingCard.hand("2c 2s"), kickers: PlayingCard.hand("3c 4h 5s"))
        let pair2 = HandEvaluationResult(rank: .pair, primary: PlayingCard.hand("2c 2s"), kickers: nil)
        XCTAssertEqual(pair1, pair2)
        XCTAssertEqual(pair2, pair1)
        XCTAssertEqual(pair1, HandEvaluationResult())

        let quads1 = HandEvaluationResult(rank: .fourOfKind, primary: PlayingCard.hand("4c 4s 4h 4d"), kickers: PlayingCard.hand("ah"))
        let quads2 = HandEvaluationResult(rank: .fourOfKind, primary: PlayingCard.hand("4c 4s 4h 4d"), kickers: PlayingCard.hand("2h"))
